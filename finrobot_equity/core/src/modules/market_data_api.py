...
def fetch_yf_income_statement(ticker: str, limit: int = 5) -> pd.DataFrame | None:
    try:
        t = yf.Ticker(ticker)
        fin = t.financials
        if fin is None or fin.empty:
            return None
        fin_t = fin.T.reset_index()
        fin_t.rename(columns={fin_t.columns[0]: 'date'}, inplace=True)
        fin_t['date'] = pd.to_datetime(fin_t['date'])
        fin_t['year'] = fin_t['date'].dt.year
        fin_t['symbol'] = ticker
        ...
        for yf_c, fmp_c in mapping.items():
            if yf_c in fin_t.columns:
                fin_t[fmp_c] = fin_t[yf_c]
        
        for col in ['revenue', 'costOfRevenue', 'grossProfit', 'operatingIncome', 'ebitda', 'netIncome', 'researchAndDevelopmentExpenses', 'sellingGeneralAndAdministrativeExpenses']:
            if col not in fin_t.columns:
                fin_t[col] = 0.0
            else:
                fin_t[col] = fin_t[col].fillna(0.0)

        if (fin_t['ebitda'] == 0).all():
            fin_t['ebitda'] = fin_t['operatingIncome']
            
        return fin_t.sort_values(by='date', ascending=False).head(limit)
    except Exception as e:
        print(f"Error fetching yfinance income statement for {ticker}: {e}")
        return None

def fetch_yf_balance_sheet(ticker: str, limit: int = 5) -> pd.DataFrame | None:
    try:
        t = yf.Ticker(ticker)
        bs = t.balance_sheet
        if bs is None or bs.empty:
            return None
        bs_t = bs.T.reset_index()
        bs_t.rename(columns={bs_t.columns[0]: 'date'}, inplace=True)
        bs_t['date'] = pd.to_datetime(bs_t['date'])
        bs_t['year'] = bs_t['date'].dt.year
        bs_t['symbol'] = ticker
        return bs_t.sort_values(by='date', ascending=False).head(limit)
    except Exception as e:
        print(f"Error fetching yfinance balance sheet for {ticker}: {e}")
        return None

def fetch_yf_cash_flow(ticker: str, limit: int = 5) -> pd.DataFrame | None:
    try:
        t = yf.Ticker(ticker)
        cf = t.cashflow
        if cf is None or cf.empty:
            return None
        cf_t = cf.T.reset_index()
        cf_t.rename(columns={cf_t.columns[0]: 'date'}, inplace=True)
        cf_t['date'] = pd.to_datetime(cf_t['date'])
        cf_t['year'] = cf_t['date'].dt.year
        cf_t['symbol'] = ticker
        return cf_t.sort_values(by='date', ascending=False).head(limit)
    except Exception as e:
...
def get_fmp_income_statement(ticker: str, api_key: str, period: str = "annual", limit: int = 5) -> pd.DataFrame | None:
    ...
    if api_key:
        try:
            ...
            print(f"Error fetching FMP income statement for {ticker}: {e}, falling back to yfinance")
    return fetch_yf_income_statement(ticker, limit)
...
def get_fmp_balance_sheet(ticker: str, api_key: str, period: str = "annual", limit: int = 5) -> pd.DataFrame | None:
    ...
    if api_key:
        try:
            ...
            print(f"Error fetching FMP balance sheet for {ticker}: {e}, falling back to yfinance")
    return fetch_yf_balance_sheet(ticker, limit)
...
def get_fmp_cash_flow_statement(ticker: str, api_key: str, period: str = "annual", limit: int = 5) -> pd.DataFrame | None:
    ...
    if api_key:
        try:
            ...
            print(f"Error fetching FMP cash flow for {ticker}: {e}, falling back to yfinance")
    return fetch_yf_cash_flow(ticker, limit)
