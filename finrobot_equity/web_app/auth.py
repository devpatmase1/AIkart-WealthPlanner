...
def get_current_user(request: Request) -> Optional[Dict]:
    """Get current user from session cookie, or default local admin user if no session cookie exists."""
    ...
    try:
        ...
        # Bypass local login requirement by using existing default admin user
        local_user = crud.get_user_by_id(db, 1)
        if local_user:
            return {
                "id": local_user.id,
                "email": local_user.email,
                "name": local_user.name,
                "provider": local_user.provider,
...
def init_default_admin():
    ...
    try:
        ...
        if stats["total_users"] == 0:
            ...
            print(f"[OK] Created default admin user: {admin_email}")
            if not os.getenv("FINROBOT_ADMIN_PASSWORD"):
                print(f"[WARNING] Generated admin password: {admin_password}")
